...
class TwoScalar:
    ...
    def __init__(self, path):
        ...
        self.reduction_type = self._reduction_types[
            None
        ]  # All bhns files are of the 'scalar' reduction type
        ...
        if file_name_segment1 is not None:
            if file_name_segment1 == "don":
                ...
                raise RuntimeError(
                    f"Naming scheme not recognized for {filename}"
                )
        ...
        elif (file_name_segment3 == "jon") and (
            extraction_radius_number is not None
        ):
            ...
            raise RuntimeError(f"Naming scheme not recognized for {filename}")
